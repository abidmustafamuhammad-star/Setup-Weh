...
const { genToken, sanitize, formatRp, getIp, applyMarkup, safeEqual, log } = require('../lib/helpers');
...
// ============================================================
// PERBAIKAN CORS
// Sebelumnya: cors({ origin: true, credentials: true }) — ini me-reflect
// header Origin APA PUN sebagai diizinkan. Digabung dengan credentials:true,
// artinya situs mana pun bisa memicu request dengan cookie session korban
// dan membaca hasilnya (celah CSRF/credential-leak besar).
// Karena frontend selalu same-origin dengan API (lihat vercel.json rewrites),
// origin browser normal tidak akan mengirim header Origin sama sekali untuk
// same-origin request, jadi whitelist ketat ini tidak mengganggu penggunaan normal.
// ============================================================
const allowedOrigins = new Set([config.server.baseUrl, ...config.cors.origins]);
app.use(cors({
  origin(origin, cb) {
    if (!origin) return cb(null, true); // same-origin browser request / server-to-server (webhook)
...
app.use((req, res, next) => {
  ...
  if (config.server.secureCookie) {
    res.setHeader('Strict-Transport-Security', 'max-age=63072000; includeSubDomains');
  }
  // CATATAN: 'unsafe-inline' masih dibutuhkan karena main.html/admin.html/login.html
  // memakai banyak inline <script> dan onclick="". Ini tetap menutup vector paling umum
  // (object embed, clickjacking, base-uri hijack, load script dari domain asing).
  // Untuk CSP yang lebih ketat, perlu refactor onclick -> addEventListener di file JS terpisah.
...
// ============================================================
// PERBAIKAN RATE LIMIT
// Sebelumnya pakai Map() in-memory — tidak reliable di serverless (lihat lib/rateLimit.js).
// ============================================================
app.use('/api/', async (req, res, next) => {
  try {
    const db = await getDb();
    const ip = getIp(req);
    const r = await rateLimit.hit(db, `req:${ip}`, config.security.rateLimitMax, config.security.rateLimitWindow);
    if (r.limited) return res.status(429).json({ success: false, error: 'Terlalu banyak request. Coba lagi sebentar.' });
    next();
  } catch (e) {
    log.error('rate limit middleware', e.message);
    next(); // fail-open: gangguan DB rate-limit tidak boleh menjatuhkan seluruh API
  }
});
...
// PERBAIKAN RACE CONDITION ADMIN PERTAMA
// Sebelumnya: role = (countDocuments()===0) ? 'admin' : 'user'. Ini TIDAK atomik —
// dua request register yang datang bersamaan bisa sama-sama melihat count=0 dan
// sama-sama jadi admin, atau sebaliknya penyerang yang register duluan mencuri slot admin.
// insertOne dengan _id unik itu atomik di level MongoDB, jadi hanya SATU request yang bisa menang.
async function claimFirstAdminSlot(db) {
  try {
    await db.collection('counters').insertOne({ _id: 'admin_bootstrap_claimed', at: new Date() });
    return true; // request ini yang pertama & sah mengklaim slot admin
  } catch (e) {
    if (e.code === 11000) return false; // slot sudah diklaim request lain
    throw e;
  }
}
...
// Dipakai untuk menyamakan waktu respons login antara "user tidak ada" vs "password salah"
// (lihat PERBAIKAN TIMING ATTACK di bawah).
const DUMMY_HASH = bcrypt.hashSync('dummy-password-for-timing-safety-only', config.security.bcryptRounds);
...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    ...
    const isFirstAdmin = await claimFirstAdminSlot(db); // PERBAIKAN: atomik, lihat definisi di atas
...
app.post('/api/auth/admin-setup', async (req, res) => {
  ...
  try {
    ...
    // PERBAIKAN: pakai safeEqual (constant-time) — sebelumnya `!==` biasa rawan timing attack.
    if (!setup_key || !safeEqual(setup_key, config.security.adminSetupKey)) {
...
app.post('/api/auth/login', async (req, res) => {
  ...
  const ip = getIp(req);
  try {
    // PERBAIKAN: lockout login sekarang atomik via MongoDB (lihat lib/rateLimit.js),
    // bukan Map in-memory yang reset tiap cold start di serverless.
    const attempt = await rateLimit.hit(db, `login:${ip}`, config.security.maxLoginAttempts, config.security.lockoutDuration);
    if (attempt.limited) {
      const wait = Math.ceil((attempt.resetAt - Date.now()) / 60000);
    ...
    // PERBAIKAN TIMING ATTACK: sebelumnya bcrypt.compare() hanya dipanggil kalau user
    // ditemukan, jadi respons "user tidak ada" jauh lebih cepat daripada "password salah" —
    // penyerang bisa menebak email/username terdaftar lewat selisih waktu respons.
    // Sekarang bcrypt.compare selalu dijalankan (terhadap dummy hash kalau user tidak ada).
    const valid = user
...
app.get('/api/dashboard', authMiddleware, async (req, res) => {
  try {
    ...
    const [activeCount, completedCount] = await Promise.all([
      db.collection('otp_orders').countDocuments({ user_id: u._id, status: { $in: ['ACTIVE', 'OTP_RECEIVED'] } }),
      db.collection('otp_orders').countDocuments({ user_id: u._id, status: 'COMPLETED' }),
    ]);
    res.json({
      success: true,
      stats: {
        ...
        active_orders: activeCount, completed_orders: completedCount,
      },
  ...
  try {
    ...
    if (!verified) {
      await db.collection('webhook_logs').updateOne(
        ...
        { $set: { processed: false, verify_failed: true, verify_failed_at: new Date() } }
      );
      await auditLog(db, null, 'webhook_pakasir_verify_failed', { order_id, amount }, ip, 'failed');
      return res.status(400).json({ success: false, error: 'Verifikasi transaksi gagal' });
    }
...
app.all('/api/cron/expire-orders', async (req, res) => {
  try {
    ...
    // PERBAIKAN: constant-time compare (lihat safeEqual di lib/helpers.js).
    if (!config.security.cronSecret || !safeEqual(secret, config.security.cronSecret)) {
...
// ============================================================
// 404 — dipisah antara API (JSON) dan halaman (HTML) — lihat public/404.html
// ============================================================
app.use((req, res) => {
